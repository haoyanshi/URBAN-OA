...
summary(Prevalence2021_ASR$val)
##3.2 确定色阶 (A.确定节点breaks；B.根据节点形成区间break_labels; C.给不同的区间加上不同的颜色 pal)####
###3.2.1 确定节点breaks####
breaks <- c(5000,5500,6000,6500,7000,7500,8000,8500,9000)
...
Prevalence2021_ASR_map_plot <- ggplot(data = Prevalence2021_ASR_map,aes(fill = val2)) +
  ...
  theme_void()+
  labs(x="", y="")+
  guides(fill = guide_legend(title='ASPR in 2021 per 100,000 persons', ncol =2))+
...
###4.1.1 准备作图数据####
a <- Prevalence2021_ASR_map[Prevalence2021_ASR_map$location_name %in% subregions_shp[['Caribbean and central America']]$Location.Name,]
...
## 4.2 Persian Gulf ####
###4.2.1 准备作图数据
a <- Prevalence2021_ASR_map[Prevalence2021_ASR_map$location_name %in% subregions_shp[['Persian Gulf']]$Location.Name,]
...
a <- Prevalence2021_ASR_map[Prevalence2021_ASR_map$location_name %in% subregions_shp[['Balkan Peninsula']]$Location.Name,]
...
## 4.4 Southeast Asia ####
a <- Prevalence2021_ASR_map[Prevalence2021_ASR_map$location_name %in% subregions_shp[['Southeast Asia']]$Location.Name,]
...
## 4.5.West Africa ####
a <- Prevalence2021_ASR_map[Prevalence2021_ASR_map$location_name %in% subregions_shp[['West Africa']]$Location.Name,]
...
a <- Prevalence2021_ASR_map[Prevalence2021_ASR_map$location_name %in% subregions_shp[['Eastern Mediterranean']]$Location.Name,]
...
## 4.7.Northern Europe ####
a <- Prevalence2021_ASR_map[Prevalence2021_ASR_map$location_name %in% subregions_shp[['Nothern Europe']]$Location.Name,]
...
Prevalence2019_ASR_map_plot <- Prevalence2021_ASR_map_plot / plot3 + plot_layout(height = c(2,1),widths = c(2,1))
```
Prevalence2019_ASR_map_plot

#6.保存
```{R}
ggsave(Prevalence2019_ASR_map_plot, file = 'Prevalence2019_ASR_map.pdf', units = 'cm', width = 24, height = 17.8)
